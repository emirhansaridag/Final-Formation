...
func _ready():
	...
	health = max_health
	speed = config.enemy_move_speed * 0.6  # Slower but tankier
	...
	scale = config.burak_boss_scale  # Use dedicated Burak Boss scale
	
	# Store this scale as the "original" for pooling
	set_meta("original_scale", scale)
	
	# Debug: Print boss info
	print("🔥🔥 BURAK BOSS spawned - Health: ", max_health, " Speed: ", speed, " Scale: ", scale)
	
	# Start Burak Boss animation
	start_animation()
...
func _on_death():
	...
	print("🎉🔥 BURAK BOSS DEFEATED! Epic victory!")
	_cleanup_enemy()
...
func start_animation():
	...
	if animation_player:
		animation_player.play("run3")  # Burak Boss animation
		# In Godot 4, we get the animation and set its loop mode
		var animation_library = animation_player.get_animation_library("")
		if animation_library and animation_library.has_animation("run3"):
			var animation = animation_library.get_animation("run3")
			animation.loop_mode = Animation.LOOP_LINEAR
		print("🔥 Burak Boss animation 'run3' started")
	else:
		print("Warning: No AnimationPlayer found for Burak Boss")
