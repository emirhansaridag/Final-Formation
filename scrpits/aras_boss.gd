...
var boss_damage_multiplier: float = 2.5
var boss_health_multiplier: float = 4.0
...
func _ready():
	...
	max_health = config.enemy_base_health * boss_health_multiplier  # 4x health for Aras Boss
	health = max_health
	speed = config.enemy_move_speed * 0.9  # Slightly slower than regular enemies
	...
	scale = config.aras_boss_scale  # Use dedicated Aras Boss scale
	
	# Store this scale as the "original" for pooling
	set_meta("original_scale", scale)
	
	# Debug: Print boss info
	print("🔥 Aras Boss spawned - Health: ", max_health, " Speed: ", speed, " Scale: ", scale)
	
	# Start Aras Boss animation
	start_animation()
...
func start_animation():
	...
	if animation_player:
		animation_player.play("run")  # Aras Boss animation
		...
		print("Aras Boss animation 'run' started")
	else:
		print("Warning: No AnimationPlayer found for Aras Boss")
